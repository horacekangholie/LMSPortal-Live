...
Partial Class _Default
    ...
    Protected Sub BindGridView1()
        ...
        Dim sqlStr() As String = {"SELECT * FROM D_DMC_Billed_Account_Expired_In_2_Months ORDER BY [End Date], [Bill entity], [HQ Code], [Store Code]",
                                  ...
                                  "SELECT * FROM D_Licence_With_Term WHERE [Expired Date] BETWEEN DATEADD(mm, DATEDIFF(mm, 0, GETDATE()) - 12, 0) AND DATEADD (dd, -1, DATEADD(mm, DATEDIFF(mm, 0, GETDATE()) + 3, 0)) AND [Application Type] NOT IN ('PC Scale (AI)') AND Chargeable NOT IN ('No') ORDER BY [Expired Date] DESC ",
                                  "SELECT * FROM D_Licence_With_Term WHERE [Expired Date] <= DATEADD (dd, -1, DATEADD(mm, DATEDIFF(mm, 0, GETDATE()) + 10, 0)) AND [Application Type] IN ('PC Scale (AI)') AND [Status] NOT IN ('Renew', 'Blocked', 'Expired') AND Replace([Licence Code], '-', '') NOT IN (SELECT Replace(Value_1, '-', '') FROM DB_Lookup WHERE Lookup_Name = 'Production Used Licence Key') ORDER BY [Expired Date], [Serial No] ",
                                  "SELECT * FROM R_LMS_Module_Licence WHERE [status] IN ('Renew') ORDER BY CAST([Expired Date] AS date) DESC ",
                                  "SELECT * FROM D_Licence_With_Term WHERE [Expired Date] <= DATEADD (dd, -1, DATEADD(mm, DATEDIFF(mm, 0, GETDATE()) + 10, 0)) AND [Application Type] IN ('PC Scale (AI)') AND [Status] IN ('Expired') AND Replace([Licence Code], '-', '') NOT IN (SELECT Replace(Value_1, '-', '') FROM DB_Lookup WHERE Lookup_Name = 'Production Used Licence Key') ORDER BY [Expired Date], [Serial No] ",
